...
public class BankServiceImpl implements BankService {
    ...
    public void transferAmount(Connection connection, long accountNumberFrom, long accountNumberTo, long amount){
        ...
        int result = accountRepository.withdraw(connection, accountNumberFrom, amount);
        if(result < 1){
            throw new RuntimeException(String.format("fail-withdraw: %d", accountNumberFrom));
        }
        result = accountRepository.deposit(connection, accountNumberTo, amount);
        if(result < 1){
            throw new RuntimeException(String.format("fail-deposit: %d", accountNumberTo));
        }
    }
    ...
    public void dropAccount(Connection connection, long accountNumber) {
        ...
        if(result < 1){
            throw new RuntimeException(String.format("fail-dropAccount: %d", accountNumber));
        }
    }
